  ...
  try {
    ...
    if (config.deleteDataFiturs) {
      ...
      for (let i = 0; i < allFitur.length; i++) {
        ...
        // logging("warning", ">>> FITUR", fiturSebelumnya[fitur.id]);
        if (!fiturSebelumnya[fitur.id]) {
          logging("warning", ">>> FITUR UNDEFINED", fiturSebelumnya[fitur.id]);
          startIndexMotong = i;
        }
      }
      ...
      const cacheUpdate = cacheFiturs.concat(fiturBaru);
      ...
      if (fiturBaru.length !== allFitur.length) {
        logging("error", "masuk ke insert", {
          fiturBaru: fiturBaru.length,
        ...
        fs.writeFile(
          ...
          function (err) {
            ...
            logging(
              "success",
              "Keterangan",
              "DATA FITUR DELETE AND REPLACES !"
            );
          }
        );
        db.Fitur.bulkCreate(fiturBaru);
      }
    }
  } catch (err) {
